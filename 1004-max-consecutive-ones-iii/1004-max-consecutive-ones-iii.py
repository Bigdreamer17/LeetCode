class Solution:
    def longestOnes(self, nums: List[int], k: int) -> int:
        """
        have two pointers left and right 
        have two variables one to track if right has passed k amounts of zeros
        another to count the length of the maximum sub-array when the zeros are flipped
        have the left move until the variable that tracks the zeros until exactly to k
        then add that to new array and return the maxmium of the array
        """
        left = right = 0
      
        for right in range(len(nums)):
        # if we encounter a 0 the we decrement K
            if nums[right] == 0:
                k -= 1
        # else no impact to K
        
        # if K < 0 then we need to move the left part of the window forward
        # to try and remove the extra 0's
            if k < 0:
          # if the left one was zero then we adjust K
                if nums[left] == 0:
                    k += 1
          # regardless of whether we had a 1 or a 0 we can move left side by 1
          # if we keep seeing 1's the window still keeps moving as-is
                left += 1
      
        return right - left + 1
            
                